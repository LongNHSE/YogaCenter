...
CREATE TABLE blogPost (
	...
	[maTrainer] NVARCHAR(10) NULL,
	[maCate] NVARCHAR(10)  NULL,
	[status] BIT NOT NULL DEFAULT 0,
	CONSTRAINT fk_maHV_Blog FOREIGN KEY ([maHV]) REFERENCES hocVien([maHV]),
	CONSTRAINT fk_maTrainer_Blog FOREIGN KEY ([maTrainer]) REFERENCES Trainer([maTrainer]),
...
CREATE TABLE [application](
	...
	[maApplicationType] nvarchar(10) not null,
	[noiDung] NVARCHAR(255) NOT NULL,
	[Date] date not null,
	[status] nvarchar(15) not null
	CONSTRAINT fk_maHV_application2  FOREIGN KEY ([maHV]) REFERENCES hocVien([maHV]),
	CONSTRAINT fk_maTrainer_application2 FOREIGN KEY ([maTrainer]) REFERENCES Trainer([maTrainer]),
	constraint fk_maLopHoc_application2 foreign key([maLopHoc]) references [lopHoc]([maLopHoc]),
	CONSTRAINT fk_applicationType_application2 FOREIGN KEY ([maApplicationType]) REFERENCES applicationType([maApplicationType])
	)
	Create table applicationType(
	[maApplicationType] nvarchar(10) primary key,
	[tenApplication] nvarchar(20) not null
	)
	Create table blogComment(
	[commentID] nvarchar(10) primary key,
	[commentContent] nvarchar(max) not null,
	[maBlog] NVARCHAR(10) not null, --constraint
	[maHV] NVARCHAR(10) not null, --constraint
	[maTrainer] NVARCHAR(10) not null, --constraint
	CONSTRAINT fk_maBlog_application2  FOREIGN KEY ([maHV]) REFERENCES hocVien([maHV]),
	CONSTRAINT fk_maHV_comment  FOREIGN KEY ([maHV]) REFERENCES hocVien([maHV]),
	CONSTRAINT fk_maTrainer_comment  FOREIGN KEY ([maTrainer]) REFERENCES Trainer([maTrainer]),
	)
	ALTER TABLE blogPost
ADD [ngayCapNhat] DATE null;
